...
model{
  ...
  for (i in 1:length(ai.prox)){
    ...
    d18Of[i] <- d18Ofpdb[i]*(1-indexop) + d18Oseccal*indexop
  ...
  tempC.phi ~ dbeta(5,2) 
  tempC.eps[1] = 0 
  tempC.tau ~ dgamma(300, 10) #changed from dgamma(10,10)
  
  # [Ca] (mmol kg^-1)
  xca[1] ~ dnorm(xca.m, xca.p)
  xca.phi ~ dbeta(5,2) 
  xca.eps[1] = 0 
  xca.tau ~ dgamma(1e3, 1e-4) #changed from dgamma(1e3,1e-4)
  ...
  for (i in 2:n.steps){
    ...
    tempC.eps[i] ~ dnorm(tempC.eps[i-1]*(tempC.phi^dt[i-1]), tempC.pc[i])T(-6, 6) # changed from T(12,12)
    ...
    d18Osw.eps[i] ~ dnorm(d18Osw.eps[i-1]*(d18Osw.phi^dt[i-1]), d18Osw.pc[i])T(-2, 2) # changed from T(-0.2,-0.2)
